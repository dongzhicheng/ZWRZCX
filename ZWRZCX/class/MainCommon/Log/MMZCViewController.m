...
-(void)createLabel{
    UILabel *label=[[UILabel alloc]initWithFrame:CGRectMake((self.view.frame.size.width-140)/2, 360, 140, 21)];
...
-(void)createButtons{
    ...
    //微信
    weixinBtn=[[UIButton alloc]initWithFrame:CGRectMake((self.view.frame.size.width-50)/2, 380, 50, 50)];
    ...
    //qq
    QQBtn=[[UIButton alloc]initWithFrame:CGRectMake((self.view.frame.size.width-50)/2-100, 380, 50, 50)];
    ...
    //新浪微博
    xinlangBtn=[[UIButton alloc]initWithFrame:CGRectMake((self.view.frame.size.width-50)/2+100, 380, 50, 50)];
...
#pragma mark - 工具
-(NSString*)getHiddenStringWithPhoneNumber:(NSString*)number middle:(NSInteger)countHiiden{ //手机号格式化
...
-(NSString*)getHiddenStringWithPhoneNumber1:(NSString*)number middle:(NSInteger)countHiiden{ //手机号格式化后还原
    if (number.length<countHiiden) {
        return number;
    }
    NSString *xings=@"";

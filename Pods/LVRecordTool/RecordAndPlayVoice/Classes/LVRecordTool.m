...
@property (nonatomic, copy)NSString * strRecord;
...
- (AVAudioRecorder *)recorder {
    if (!_recorder) {
        ...
        NSString *path = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject]; // 获取沙盒地址
        NSString *filePath = [path stringByAppendingPathComponent:LVRecordFielName];
        self.recordFileUrl = [NSURL fileURLWithPath:filePath];
        NSLog(@"filePath-->>%@", filePath);
        
        NSMutableDictionary *setting = [NSMutableDictionary dictionary];  // 设置录音的一些参数
